...
The sunburst plot provides an at-a-glance distribution and hierarchy of pediatric cancer diagnoses, with all PeCan samples mapped to a custom ontology (Figure 2).
...
This will enable custom interface views per data facet based on the sunburst selection. Alternatively, a user can explore all of the available data within a data facet by clicking directly icon from the side navigation.
Metadata export is also available via the download all metadata option.
...
The Subtype Tree allows users to explore the full diagnostic classification structure used across St. Jude Cloud data, from broad categories down to individual subtypes.
...
The tree can be downloaded as an SVG for external use, and all available PeCan metadata is available for export via the Download all metadata option.
