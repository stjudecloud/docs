...
![image 1](/img/visualization-community/proteinpaint/using-custom-tracks/image1.png)
...
- "**url**": STR
- "**indexURL**": STR
  - Either "file" or "url" should be provided, but not both.
    When using "file", provide the relative path to the track file starting from `<TP>` directory as is configured on the ProteinPaint server.
  - When using URL for tabix-indexed files, by default it requires the index file to share URL with the .gz file.
    When it's not sharing the URL, the attribute "indexURL" must be used to provide the URL of the index file.
- "**toppad**": INT
...
Live example: [https://proteinpaint.stjude.org/?appcard=bigwig](https://proteinpaint.stjude.org/?appcard=bigwig).
...
- **dotplotfactor**
  - Value is positive integer e.g. 5 or 10. When applied, will request 5 or 10 times more data points from a bigWig track and plot each point as a dot, rather than bars.
    A use case is checking (large-scale) CNV from DNA sequencing coverage track

![image 3](/img/visualization-community/proteinpaint/using-custom-tracks/image3.png)

### Track: a pair of bigWig for data from forward and reverse strands

Example: [https://proteinpaint.stjude.org/?appcard=bigwig](https://proteinpaint.stjude.org/?appcard=bigwig)
...
![image 4](/img/visualization-community/proteinpaint/using-custom-tracks/image4.png)
...
Example: [https://proteinpaint.stjude.org/?appcard=bedj](https://proteinpaint.stjude.org/?appcard=bedj)
...
Example: [https://proteinpaint.stjude.org/?appcard=pgv](https://proteinpaint.stjude.org/?appcard=pgv)
...
Example: [https://proteinpaint.stjude.org/?appcard=junction](https://proteinpaint.stjude.org/?appcard=junction)
...
Example: [https://proteinpaint.stjude.org/?appcard=BCF](https://proteinpaint.stjude.org/?appcard=BCF)
...
![image 2](/img/visualization-community/proteinpaint/using-custom-tracks/image2.png)
...
Bampile track can be used to examine the very rare alleles from a high-depth (capture-based) DNA sequencing experiment.
Example of bampile track:
...
Example: [https://proteinpaint.stjude.org/?appcard=ai](https://proteinpaint.stjude.org/?appcard=ai)
...
Example: [https://proteinpaint.stjude.org/?appcard=genomepaint](https://proteinpaint.stjude.org/?appcard=genomepaint)

This launches **custom** MDS track (aka GenomePaint).
To access official tracks, see [embedding API](https://docs.google.com/document/d/1KNx4pVCKd4wgoHI4pjknBRTLrzYp6AL_D-j6MjcQSvQ/edit#heading=h.jibbdhrfuw7f).
...
Example: [https://proteinpaint.stjude.org/?appcard=ase](https://proteinpaint.stjude.org/?appcard=ase)
...
This track can be built standalone, or spawned from a multi-sample track (a special mode of mdssvcnv track)
