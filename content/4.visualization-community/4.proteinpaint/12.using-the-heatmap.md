...
![image 2](/img/visualization-community/proteinpaint/using-the-heatmap/image2.png)

The heatmap can be customized to great extent.
Click the button "advanced" on top:

![image 7](/img/visualization-community/proteinpaint/using-the-heatmap/image7.png)

A box opens on the left showing texts that can be edited.
The content is an JSON object describing the heatmap.
Make modification to the JSON content, click "update" button to render a new heatmap to reflect your changes.

::callout{icon="i-heroicons-exclamation-circle" color="orange"}
**"Save your map before leaving..."**  
ProteinPaint will not save the JSON when you leave, because the heatmap is based on user-provided data.
If you have made edits, copy and save the JSON text before leaving ProteinPaint.
Next time you explore the same dataset, launch the heatmap, and paste your saved JSON in the text box to re-create the heatmap.
::

::callout{icon="i-heroicons-light-bulb" color="blue"}
**"Show a preconfigured map"**  

1. A preconfigured map can be stored in the schematics of a study view.
   [Refer to the "**heatmapJSON**" attribute of the study schematics](https://drive.google.com/open?id=121SsSYiCb3NCU8jz0bF7UujFSN-1Y20b674dqa30iXE)
...
By applying the "sortorder" attribute to a gene, samples will be sorted by the descending order of number of mutations in this gene.
Samples with more mutation in this gene will be moved to left (or top).

Value is any number.

Multiple genes can be used for sorting.
In such case, the numerical value determines the precedence of which genes are taken into sorting (smaller the value, earlier the gene is used).
...
To manually adjust sample order, the sorting-by-gene must be disabled first.
Then edit the order of samples in the sample group section of the heatmap JSON to take effect.

### Sample group

Samples can be arranged into groups.
Space can be applied between groups and borders can be drawn around each group, so that they appear distinct visually.
...
You can do this by clicking the "refresh sample" button located on top, rather than editing the JSON.
This is because by default the heatmap chooses samples by requiring the sample to have at least 1 mutation in the initial set of genes.
Afterwards the set of samples stay unchanged unless you alter the "samplegroup" contents in JSON.

Sometime when user started using a new gene set different from the initial set, he/she would like the samples to update accordingly.
This is when the "refresh sample" button comes in handy.
...
Add border attribute to either a sample or gene group, and a solid border in specified color will be drawn enclosing this group.

Optionally provide "borderwidth" attribute to specify the width using integer value.
Default 1.
...
Applicable for both data cells and metadata cells.
...
Add background attribute to a sample group.
The entire group will show this background color.
Default not applied.
...
![image 3](/img/visualization-community/proteinpaint/using-the-heatmap/image3.png)
...
Add the **cellbg** attribute to a sample.
All cells belonging to this sample will use the specified background color instead of the global background color.
...
This is deprecated.
Try to use "**metadatabymatrix**" instead.
...
![image 4](/img/visualization-community/proteinpaint/using-the-heatmap/image4.png)
...
Value is a list, with each metadata term as one element.
A term is defined as an object like below:
...
values: attribute values for this term.
Key is attribute name, value is rendering color.
...
When there IS sampletype, using this attribute will annotate patients, but not the separate samples.
To annotate samples instead (e.g. diagnosis/relapse), use "sampleannotation" described below.
...
![image 5](/img/visualization-community/proteinpaint/using-the-heatmap/image5.png)
![image 1](/img/visualization-community/proteinpaint/using-the-heatmap/image1.png)

Example of a matrix viewed in spreadsheet:

![image 9](/img/visualization-community/proteinpaint/using-the-heatmap/image9.png)
...
![image 6](/img/visualization-community/proteinpaint/using-the-heatmap/image6.png)
