...
Somatic VCF files available on St. Jude Cloud have been generated when possible for tumor samples where a matching normal sample has also been sequenced. These include samples from the Pediatric Cancer Genome Project (PCGP), Clinical Pilot, Genomes 4 Kids (G4K), and Real-time Clinical Genomics (RTCG) datasets (https://platform.stjude.cloud/data/cohorts/pediatric-cancer). These somatic VCF files contain HG38 coordinate SNV/Indel variant calls generated by the St. Jude somatic variant analysis pipeline outlined below:

1. WGS and WES reads were aligned to HG19 using [bwa backtrack][bwa] (`bwa aln` + `bwa sampe`) using default parameters.
